...
describe("Application", () => {
  ...
  /* The test below is failing. It isn't updating the spots remaining when deleting interview */
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    const appointment = getAllByTestId(container, "appointment").find((spot) =>
      queryByText(spot, "Archie Cohen")
    );
    ...
    await waitForElement(() => container, "Sylvia Palmer");
    expect(getByText(container, "Sylvia Palmer"));

    // 7. Save the data
    fireEvent.click(getByText(appointment, "Save"));

    await waitForElement(() => container, "Leopold Silvers");
    expect(queryByText(container, "Leopold Silvers")).toBeInTheDocument();
